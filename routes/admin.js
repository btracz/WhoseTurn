...
var auth = function (req, res, next) {
  ...
  if (
    user.name === config.getUserAdmin() &&
    user.pass === config.getPasswordAdmin()
  ) {
    return next();
  } else {
    return unauthorized(res);
...
router.post("/parameters", auth, function (req, res) {
  ...
  configToSave.dayOfOccurrence = Number.parseInt(data.dayOfOccurrence || "5");
...
router.get("/poll", auth, function (req, res) {
  ...
  if (openedPoll) {
    ...
    openedPoll.respondents.forEach(function (resp) {
      ...
      resp.dateText = resp.answerDate
        ? moment(resp.answerDate)
            .tz("Europe/Paris")
...
router.get("/send-notification", auth, function (req, res) {
  mailer.sendWeeklyNotification().then(
    function (value) {
