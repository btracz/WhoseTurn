...
const { ERRORS } = require("../src/constants.js");
...
router.get("/", function (req, res, next) {
  try {
    ...
    if (err.message === ERRORS.NO_USER_ERROR) {
      res.render("init", { title: "Initialisation incomplète", model: model });
    }
  }
});
...
router.post("/search", function (req, res, next) {
  ...
  users.getUsers().forEach(function (user) {
    if (reg.test(user.id) && user.hasSubscribe == true) {
      user.date = (
        _.last(_.where(planning.getPlanningOnly(), { deliverer: user.id })) ||
        {}
      ).date;
